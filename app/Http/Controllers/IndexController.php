...
class IndexController extends Controller
{
 ...
 public function deltender($id)
 {
  ...
  if ($validator->fails()) {
    ...
    {
        $name = Tender::find($id);
         $c = Bid::where('user_id','=',Auth::user()->id)->where('tender_id','=',$id)->count();
     ...
     public function getProposals($id)
          ...
          $c = Bid::where('tender_id',"=",$id)->count();

          return view('proposals')->with('proposals', $name)->with('tender', $tender)->with('count', $c);
                ...
                return view('200t');
                 ...
                 //foreach($user as $key){
                   $c_name = $user->c_name;
                    ...
                    $tender_obj->id = $t_id;
                    $tender = Tender::findOrFail($tender_obj->id); // Eloquent Model
                    $tender->update(['status' => "closed",'awarded_to' => $id,'c_name' => $c_name]);
                 //}
                 $text = "You have been awarded Bamburi contract. We will contact you for more details.";
                 $to = $user->phoneno;
                 SMSProvider::sendMessage($to, $text);
